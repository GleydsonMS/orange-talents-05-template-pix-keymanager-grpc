...
import br.com.zup.edu.compartilhado.excessoes.handlers.DefaultExceptionHandler
...
class ExceptionHandlerResolverTest {
    ...
    fun `deve retornar o ExceptionHandler especifico para o tipo de excecao`() {
        val resolved = resolver.resolve(java.lang.IllegalArgumentException())
    ...
    @Test
    fun `deve lancer um erro caso encontre mais de um ExceptionHandler que suporte a mesma excecao`() {
        resolver = ExceptionHandlerResolver(listOf(illegalArgumentExceptionHandler, illegalArgumentExceptionHandler))

        assertThrows<IllegalStateException> {
            resolver.resolve(java.lang.IllegalArgumentException())
        }
    }
}